...
Ensure skill evaluations produce fair, comparable results across models and versions.
...
- `eval_id`, `run_id` (ISO timestamp), `skill_version` (from frontmatter or "baseline"), `skill_commit`, `model`, `with_skill` (true/false), `eval_set_version`, `score` (X/Y), `overall` (pass/partial/fail), `notes`
...
If v2 only ran 4 of 7 evals from v1, flag the comparison as incomplete.
